...
public class Dashbord extends Application {
	public static String valueExit="c";
	...
	public void start(Stage primaryStage) throws Exception {
		...
		Button mcat = new Button("Manage Statistic");
		...
		rech.setOnAction(new EventHandler<ActionEvent>() {
			...
			@Override
			public void handle(ActionEvent event) {
				Dashbord.rechercheChams=rech.getText();
				RechercheAllType rm = new RechercheAllType();
				addEv.setVisible(false);
				addnews.setVisible(false);
				try {
					p.setCenter(rm.start());
					p.getCenter().setTranslateY(130);
					p.getCenter().setTranslateX(60);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
		...
		rech.setTranslateX(250);
		rech.setTranslateY(-400);
		mcat.setOnAction(new EventHandler<ActionEvent>() {
			
			@Override
			public void handle(ActionEvent event) {
				addnews.setVisible(false);
				addEv.setVisible(false);
				StatMall a = new StatMall();
				try {
		...
		VBox x = new VBox();
		x.getChildren().addAll(dec, muser, mshop, mcat, mevent, mnews, mshopr, addnews, addEv,mrequest,rech);
		p.setLeft(x);
		Main mm = new Main();
		p.setTop(mm.start());

...
public class ShopOwnerAddEvent {
	...
	public Pane start() throws Exception {
		...
		table.setPrefSize(220, 150);
		table.setTranslateX(500);
		table.setTranslateY(50);

		// colonne nom
		TableColumn<Item, String> nom = new TableColumn<>("Name Item");
		nom.setEditable(true);
		nom.setMinWidth(70);
		nom.setCellValueFactory(new Callback<CellDataFeatures<Item, String>, ObservableValue<String>>() {
		...
		// colonne discount
		TableColumn<Item, String> discount = new TableColumn<>("Discount");
		discount.setEditable(true);
		discount.setMinWidth(70);
		discount.setCellValueFactory(new Callback<CellDataFeatures<Item, String>, ObservableValue<String>>() {
			public ObservableValue<String> call(CellDataFeatures<Item, String> p) {
				variable = p.getValue().getPrice();
				return new SimpleStringProperty("");
			}
		});

		// colonne price
		TableColumn<Item, String> init = new TableColumn<>("Price");
		init.setEditable(true);
		init.setMinWidth(70);
		init.setCellValueFactory(new Callback<CellDataFeatures<Item, String>, ObservableValue<String>>() {
			public ObservableValue<String> call(CellDataFeatures<Item, String> p) {
		...
		// modifier discount
		discount.setCellFactory(TextFieldTableCell.<Item> forTableColumn());

		discount.setOnEditCommit(new EventHandler<TableColumn.CellEditEvent<Item, String>>() {

			public void handle(TableColumn.CellEditEvent<Item, String> event) {
				Item a = event.getTableView().getItems().get(event.getTablePosition().getRow());

				modifierPrice(a, Float.parseFloat(event.getNewValue()));

			}
		});

		table.setItems(listShop());
		table.getColumns().addAll(nom, discount, init);
		...
		Button save = new Button("Send request to the administrator");
		...
		save.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				ev.setShopOwner(EventServiceDelegate.searchUserByid(6));//Connexion.shopKriaa.getId_account()));// recupereerr
																		// depuis
																		// authentification
				...
				if (date1.getValue().isAfter(LocalDate.now()) && date1.getValue().isBefore(da2)) {
					...
					ev.setDateDebut(da1);
					ev.setDateFin(da3);
					int yosra = EventServiceDelegate.addEvent2(ev);
					a = yosra;
				...
				try {
					...
					dropBox.uploadToDropbox(filedropBox,ev.getShopOwner().getUsername());
				} catch (IOException e) {
	...
	public ObservableList<Item> listShop() {
		ObservableList<Item> liste = FXCollections.observableArrayList();

		List<Item> maliste = EventServiceDelegate.listItems(Connexion.shopKriaa.getId_account());
		liste.addAll(maliste);

		return liste;
	}

	public void modifierPrice(Item item, float discount) {
		System.out.println(a);
		Event ev = new Event();
		ev = EventServiceDelegate.findEvent(a);
		float b = ((100 - discount) * item.getPrice()) / 100;
		item.setPrice(b);
		item.setEvent(ev);
		item.setDiscount(discount);
		EventServiceDelegate.updateItem(item);
		table.setItems(listShop());
	}
